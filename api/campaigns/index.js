...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const status =
      ["active", "paused", "completed"].includes(body.status)
        ? body.status
        : "active";
    ...
    // ✅ Auto generate realistic values
    const impressions =
      toNumber(body.impressions) ??
      faker.number.int({ min: 10000, max: 5000000 });

    const clicks =
      toNumber(body.clicks) ??
      faker.number.int({
        min: 100,
        max: Math.floor(impressions * 0.3),
      });

    const conversions =
      toNumber(body.conversions) ??
      faker.number.int({
        min: 10,
        max: Math.floor(clicks * 0.5),
      });
